...
// Ѕуфер дл€ исходныъ путей к файлам.
...
private:
  ...
  char _fnames[(MAX_DATA_NUM+1) * MAX_PATH]; // лишнее место резервируетс€ под разделители
...
void data_window::line_sp_i( int x1, int y1, int x2, int y2 )
{
  ...
  for( int j = 0; j < endl; j++ ){
    ...
    // рисование идет черным цветом, поэтому пикселы мен€ютс€ местами
...
// возвращает -1, если опци€ не найдена
  ...
  LPSTR lpszCmdLine, int nCmdShow )
{
  ...
  // Fit, если ноль или не указано; startpos игнорируетс€
  if( scrsize == 0 )
    renderer.fit();
