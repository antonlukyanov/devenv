...
static int get_file_attr_w32( lua_State *L )
{
  const char *file = luaL_checkstring (L, 1);

  int res = GetFileAttributes(file);
  if( res == 0xFFFFFFFF ){
    LPVOID lpMsgBuf = 0;
    FormatMessage(
        ...
        NULL,
        GetLastError(),
        MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), //0, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
        (LPTSTR) &lpMsgBuf,
        0, NULL
    );

    lua_pushnil (L);
    lua_pushfstring (L, "can't get attributes for file `%s' (%s)", file, lpMsgBuf);
    LocalFree(lpMsgBuf);
    return 2;
  }
...
static int set_file_attr_w32( lua_State *L )
{
  const char *file = luaL_checkstring (L, 1);
  const char *sattr = luaL_checkstring (L, 2);
  ...
  if( !SetFileAttributes(file, attr) ){
    LPVOID lpMsgBuf = 0;
    FormatMessage(
        FORMAT_MESSAGE_ALLOCATE_BUFFER |
        FORMAT_MESSAGE_FROM_SYSTEM,
    ...
    lua_pushnil (L);
    lua_pushfstring (L, "can't set attributes for file `%s' (%s)", file, lpMsgBuf);
...
static const struct luaL_reg fslib[] = {
	...
	{"get_win32attr", get_file_attr_w32},
	{"set_win32attr", set_file_attr_w32},
	{NULL, NULL},
};
