...
public:
  ...
  virtual void func() {
    ...
    // расчет размера окна, если задана клиентска€ часть
    if( _cr ){
    ...
    // »нициализаци€ DirectDraw
    HRESULT ddrval = DirectDrawCreate(NULL, &lpDD, NULL);
    if( ddrval != DD_OK )
      DoneDD();
    if( lpDD ){
      // ѕроверим, поддерживает ли видеокарта эту функцию?
...
*/
  ...
  // workaround дл€ странного поведени€ BitBlt
