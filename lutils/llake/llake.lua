...
local function do_makefile( rep, dep, do_strip )
  local mkf = io.open('export/makefile', 'w')
  ...
  local dst = get_param('dest')
  if type(dst) == 'table' then
    for _, next_dst in pairs(dst) do
      mkf:write(next_dst, ': ', table.concat(obj_list, ' '), '\n')
      if is_param('export_link') then
        mkf:write('\t', get_param('export_link')(next_dst, obj_list), '\n')
      else
        mkf:write('\t', get_param('link')(next_dst, obj_list), '\n')
      end
      if do_strip and is_param('strip') then
        mkf:write('\t', get_param('strip')(next_dst), '\n')
      end
      mkf:write('\n')
    end
  end
...
local function do_export( rep, dep )
  if not lfs.attributes('export') and not lfs.mkdir('export') then
  ...
  for _, fi in ipairs(file_list) do
    local src =  bp..fi.dir..fi.name..fi.ext
    local dst = 'export/'..fi.name..fi.ext
    copy_file(dst, src)
  end
end
